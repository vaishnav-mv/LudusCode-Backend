...
export class GroupService implements IGroupService {
    ...
    async list(userId?: string, params: GroupListParams = {}) {
        ...
        return {
            data: groups.map(group => mapGroup(group)).filter((group) => group !== null),
            total,
            page,
    ...
    async approveJoin(groupId: string, userId: string, requesterId: string) {
        logger.debug(`[GroupService] approveJoin: groupId=${groupId}, userId=${userId}, requesterId=${requesterId}`);
        const resolvedGroupId = groupId;
        const resolvedUserId = userId; // User to approve
        logger.debug(`[GroupService] Resolved IDs: group=${resolvedGroupId}, user=${resolvedUserId}`);

        const group = await this._groups.getById(resolvedGroupId);
        if (!group) {
            logger.debug('[GroupService] Group not found');
            return false;
        }

        if (!group.pendingMembers) {
            logger.debug('[GroupService] No pendingMembers array');
            return false;
        }

        logger.debug('[GroupService] Pending members count:', group.pendingMembers.length);
        ...
        if (pendingIndex === -1) {
            logger.debug('[GroupService] User not found in pendingMembers');
            return false;
        }

        const userToApprove = group.pendingMembers[pendingIndex];
        logger.debug('[GroupService] Found user to approve:', this.getId(userToApprove as User | string));
        ...
        logger.debug(`[GroupService] Updating group. Members: ${memberIds.length}, Pending: ${pendingMemberIds.length}`);
    ...
    async rejectJoin(groupId: string, userId: string, _requesterId: string) {
        logger.debug(`[GroupService] rejectJoin: groupId=${groupId}, userId=${userId}`);
        const resolvedGroupId = groupId;
        const resolvedUserId = userId;
        ...
        const pendingIndex = group.pendingMembers.findIndex((member) => this.getId(member as User | string) === resolvedUserId);
        if (pendingIndex === -1) {
            logger.debug('[GroupService] Reject: User not found in pending');
            return false;
        }
