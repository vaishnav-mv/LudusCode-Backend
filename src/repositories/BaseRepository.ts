...
export abstract class BaseRepository<T extends Document> {
  ...
  async findById(id: string, projection?: any): Promise<T | null> {
    try {
      ...
      logger.error(
        `Error finding document by ID ${id}: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
  ...
  async findOne(query: FilterQuery<T>, projection?: any): Promise<T | null> {
    try {
      ...
      logger.error(
        `Error finding document: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
   ...
   * @throws AppError if query fails
   */
  async find(query: FilterQuery<T> = {}, projection?: any, options?: QueryOptions): Promise<T[]> {
    try {
      ...
      logger.error(
        `Error finding documents: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
  ...
  async findAll(projection?: any, options?: QueryOptions): Promise<T[]> {
    try {
      ...
      logger.error(
        `Error finding all documents: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
        `Failed to find all documents: ${error instanceof Error ? error.message : 'Unknown error'}`,
        500
      );
    }
  }
  ...
  async findPaginated(
    ...
    try {
      ...
      logger.error(
        `Error finding paginated documents: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
        `Failed to find paginated documents: ${error instanceof Error ? error.message : 'Unknown error'}`,
        500
      );
    }
  }
  ...
  async updateById(
    ...
    try {
      ...
      logger.error(
        `Error updating document by ID ${id}: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
  ...
  async deleteById(id: string): Promise<T | null> {
    try {
      ...
      logger.error(
        `Error deleting document by ID ${id}: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
  ...
  async count(query: FilterQuery<T> = {}): Promise<number> {
    try {
      ...
      logger.error(
        `Error counting documents: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
  ...
  async exists(query: FilterQuery<T>): Promise<boolean> {
    try {
      ...
      logger.error(
        `Error checking document existence: ${error instanceof Error ? error.message : String(error)}`
      );
      throw new AppError(
