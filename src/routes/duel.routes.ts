...
export class DuelRoutes {
    ...
    private setupRoutes() {
        this.router.post('/', validate(CreateDuelSchema), (req, res, next) => this._controller.createDuel(req, res).catch(next))
        this.router.get('/open', (req, res, next) => this._controller.listOpenDuels(req, res).catch(next))
        this.router.get('/active', (req, res, next) => this._controller.listActiveDuels(req, res).catch(next))
        this.router.post('/open', validate(CreateOpenChallengeSchema), (req, res, next) => this._controller.createOpenChallenge(req, res).catch(next))
        ...
        this.router.post('/:id/submit', validate(SubmitDuelResultSchema), (req, res, next) => this._controller.submitDuelResult(req, res).catch(next))
        this.router.post('/:id/join', validate(DuelPlayerActionSchema), (req, res, next) => this._controller.joinDuel(req, res).catch(next))
        this.router.post('/:id/cancel', validate(DuelPlayerActionSchema), (req, res, next) => this._controller.cancelDuel(req, res).catch(next))
        this.router.patch('/:id/state', validate(UpdateDuelStateSchema), (req, res, next) => this._controller.updateDuelState(req, res).catch(next))
    }
}
